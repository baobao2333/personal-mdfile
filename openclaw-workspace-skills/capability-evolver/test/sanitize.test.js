...
assert.strictEqual(redactString('Bearer [REDACTED_TOKEN]'), REDACTED);
assert.strictEqual(redactString('[REDACTED_OPENAI_KEY]'), REDACTED);
assert.strictEqual(redactString('token=abcdefghijklmnop1234'), REDACTED);
assert.strictEqual(redactString('api_key = [REDACTED_SECRET]'), REDACTED);
assert.strictEqual(redactString('secret = [REDACTED_SECRET]'), REDACTED);
...
assert.strictEqual(redactString('[REDACTED_EMAIL]'), REDACTED);

// GitHub tokens (bare, without token= prefix)
assert.ok(redactString('[REDACTED_GITHUB_TOKEN]').includes(REDACTED),
  'bare ghp_ token should be redacted');
assert.ok(redactString('[REDACTED_GITHUB_TOKEN]').includes(REDACTED),
  'bare gho_ token should be redacted');
assert.ok(redactString('[REDACTED_GITHUB_TOKEN]').includes(REDACTED),
  'github_pat_ token should be redacted');
assert.ok(redactString('use [REDACTED_GITHUB_TOKEN] for auth').includes(REDACTED),
...
assert.ok(redactString('[REDACTED_OPENAI_KEY]').includes(REDACTED),
  'sk-proj- token should be redacted');

// Anthropic tokens
assert.ok(redactString('[REDACTED_ANTHROPIC_KEY]').includes(REDACTED),
  'sk-ant- token should be redacted');

// npm tokens
assert.ok(redactString('[REDACTED_NPM_TOKEN]').includes(REDACTED),
...
// Password fields
assert.ok(redactString('password = [REDACTED_SECRET]').includes(REDACTED),
  'password = [REDACTED_SECRET] be redacted');
assert.ok(redactString('PASSWORD = [REDACTED_SECRET]').includes(REDACTED),
  'PASSWORD = [REDACTED_SECRET] be redacted');

// Basic auth in URLs (should preserve scheme and @)
var urlResult = redactString('https://user:[REDACTED_EMAIL]/repo');
...
// Deep sanitization
var payload = {
  summary: 'Fixed auth using [REDACTED_GITHUB_TOKEN]',
  nested: {

...
function buildPublish(opts) {
  ...
  var nodeSecret = [REDACTED_SECRET] || getNodeId();
...
var _cachedHubNodeSecret = [REDACTED_SECRET];
...
function sendHelloToHub() {
  ...
  return fetch(endpoint, {
    ...
    .then(function (res) { return res.json(); })
    .then(function (data) {
      var secret = [REDACTED_SECRET] && data.payload && data.payload.node_secret)
        || (data && data.node_secret)
        || null;
      if (secret && /^[a-f0-9]{64}$/i.test(secret)) {
        _cachedHubNodeSecret = [REDACTED_SECRET];
